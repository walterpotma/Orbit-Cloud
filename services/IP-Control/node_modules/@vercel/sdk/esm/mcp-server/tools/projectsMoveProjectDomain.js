...
export const tool$projectsMoveProjectDomain = {
...
Move one project's domain to another project. Also allows the move of all redirects pointed to that domain in the same project.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await projectsMoveProjectDomain(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
