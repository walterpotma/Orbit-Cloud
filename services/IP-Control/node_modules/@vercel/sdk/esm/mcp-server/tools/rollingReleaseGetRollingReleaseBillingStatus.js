...
export const tool$rollingReleaseGetRollingReleaseBillingStatus = {
...
Get the Rolling Releases billing status for a project. The team level billing status is used to determine if the project can be configured for rolling releases.`,
    ...
    tool: async (client, args, ctx) => {
        const [result, apiCall] = await rollingReleaseGetRollingReleaseBillingStatus(client, args.request, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
