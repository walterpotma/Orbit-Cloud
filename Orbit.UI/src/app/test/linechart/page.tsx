...
export default function ProfessionalChartPage() {
    ...
    // --- CÁLCULOS ROBUSTOS ---
    // AQUI ESTÁ A CORREÇÃO: Se width for 0, usamos um valor padrão (ex: 100%) para não quebrar a matemática
    // Mas mantemos a lógica de redimensionamento assim que o observer funcionar.
    const safeWidth = width || 0; 
    const safeHeight = height || 0;

    // Se ainda não temos tamanho, usamos 0 para evitar desenhar coisas erradas, 
    // mas não impedimos o componente de montar (o return lá embaixo acontece).
    ...
    return (
        <div className="w-full min-h-screen bg-zinc-950 flex flex-col justify-center items-center p-8 font-sans">
            ...
            {/* Adicionei overflow-hidden aqui para garantir que nada vase do card */}
            <div className="w-full max-w-3xl bg-zinc-900 rounded-xl border border-zinc-800 p-6 shadow-2xl overflow-hidden">
                ...
                {/* Container que define o tamanho FIXO da altura */}
                <div ref={containerRef} className="w-full h-[300px] relative select-none">
                    
                    {/* SVG Absoluto: Removemos a trava {width > 0 && ...} */}
                    {/* Ele renderiza sempre, mas com width=0 no primeiro milissegundo (invisível), depois cresce */}
                    <svg 
                        width={safeWidth} 
                        height={safeHeight} 
