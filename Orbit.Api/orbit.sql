-- Execute este comando separadamente. Se o banco de dados 'orbit' já existir,
-- você receberá um erro, o que é normal.
CREATE DATABASE orbit;

-- Conecte-se ao banco de dados 'orbit' antes de executar o resto do script.
-- \c orbit
...
-- CORREÇÃO: Renomeado de 'rule' para 'role' para consistência
...
-- CORREÇÃO: Tabela 'payment_method' que estava faltando foi adicionada
CREATE TABLE IF NOT EXISTS payment_method(
    Id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    AccountId INT NOT NULL,
    MethodType payment_method_enum NOT NULL,
    CardLastDigits CHAR(4), -- Exemplo de campo útil
    IsDefault BOOLEAN NOT NULL DEFAULT false,
    GatewayToken VARCHAR(255), -- Token do gateway de pagamento
    CreatedAt TIMESTAMPTZ NOT NULL DEFAULT now(),

    CONSTRAINT fk_payment_method_account FOREIGN KEY (AccountId) REFERENCES account(Id) ON DELETE CASCADE
);
...
-- CORREÇÃO: A tabela duplicada 'organization' foi removida daqui.

-- ========= ÍNDICES =========
-- CORREÇÃO: Referência a 'role' agora está correta
